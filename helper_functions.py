...
def get_alarm_list():
    choices = listdir("/home/jacob/alarm_clock/sounds")
...
def track_time(start_time, time_limit):
    ...
    #pygame.mixer.init()
    #pygame.mixer.music.load(f'/home/jacob/alarm_clock/sounds/{song_options[random]}')
...
def play_waiting_song():
    #this returns the path to the file
    #pygame.mixer.init()
    choice = pull_new_choice()
    pygame.mixer.init()
...
def play_alarm(status, choice):
    pygame.mixer.init()
    print(choice)
    pygame.mixer.music.load(f'sounds/{choice}') #plays any track as alarm if you give it the update
...
def start_threading():
    thread1 = threading.Thread(target=init_pygame_and_listen)
    thread1.start()
    
def init_pygame_and_listen(): #should on a separate thread count 3 minutes and then add a new song to the queue
    pygame.init()
...
#    pygame.mixer.music.set_endevent()
 #   if pygame.mixer.music.get_endevent() != pygame.NOEVENT:
  #      play_new_song()
def pull_new_choice():
    song_options = listdir('/home/jacob/alarm_clock/sounds')
    random = randint(0, len(song_options) - 1)
    return f'/home/jacob/alarm_clock/sounds/{song_options[random]}'
